...
class HeishamonAPI:
    ...
    async def async_get_data(self) -> dict[str, Any]:
        ...
        async with session.get(
            url, auth=self._auth, timeout=aiohttp.ClientTimeout(total=20)
        ) as response:
            response.raise_for_status()
            # HeishaMon sendet application/json, aber wir erzwingen das Parsen
            # unabhaengig vom Content-Type, damit aeltere Firmware nicht scheitert.
