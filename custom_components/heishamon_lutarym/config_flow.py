"""Config flow for Heishamon by Lutarym integration."""
...
class HeishamonConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    """Handle a config flow for Heishamon."""
    ...
    @staticmethod
    def _test_connection(host: str, username: str, password: str) -> None:
        """Test connection to Heishamon."""
        import asyncio
        
        api = HeishamonAPI(host, username, password)
        asyncio.run(api.test_connection())
